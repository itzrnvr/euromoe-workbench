...
def analyze_step():
    """Run forward pass, return analysis. Does NOT commit any token."""
    global _hooks_active, is_generating
    
    if is_generating: return {"error": "Busy"}
    is_generating = True
    ...
    if session["input_ids"] is not None and session["input_ids"].shape[1] > 2048:
        is_generating = False
        return {"error": "Context too long. Clear chat."}
    ...
    try:
        with torch.inference_mode():
            ...
            if torch.isnan(logits).any() or torch.isinf(logits).any():
                raise RuntimeError("NaN/Inf — resetting")
            
            # Predictions (top 10)
            top_vals, top_idx = torch.topk(logits.float(), 10)
            top_probs = F.softmax(logits.float(), dim=-1)[0, top_idx[0]]
            greedy_id = logits.argmax(dim=-1).item()
            preds = [
                {"t": tokenizer.decode([i.item()]), "id": i.item(), "p": float(top_probs[j].item())}
                for j, i in enumerate(top_idx[0])
            ]
            
            # KV cache
            kv_info = {"n_tokens": session["input_ids"].shape[1], "size_mb": 0}
            if outputs.past_key_values:
                total_el = sum(t.numel() for lk in outputs.past_key_values for t in lk if t is not None and hasattr(t, 'numel'))
                kv_info["size_mb"] = round(total_el * 2 / 1048576, 1)
            
            total_time = (time.perf_counter() - t_start) * 1000
            attn_total = sum(a["ms"] for a in last_step_data.get("attn_ms", []))
            moe_total = sum(a["ms"] for a in last_step_data.get("moe_ms", []))
            
            # Store logits for commit step
            session["_pending_logits"] = logits.cpu()
            session["_greedy_id"] = greedy_id
            
            del outputs
            torch.cuda.empty_cache()
            
            return {
                "greedy": {"id": greedy_id, "token": tokenizer.decode([greedy_id])},
                "preds": preds,
...
def commit_token(token_id):
    """Commit a specific token to the session."""
    session["generated_ids"].append(token_id)
    session["input_ids"] = torch.cat([
        session["input_ids"],
        torch.tensor([[token_id]], device=DEVICE)
    ], dim=-1)
    session["_pending_logits"] = None
    
    token_str = tokenizer.decode([token_id])
    full_text = tokenizer.decode(session["generated_ids"])
    
    # Check EOS
    done = (token_id == tokenizer.eos_token_id)
    if done:
        session["chat_history"][-1]["assistant"] = full_text
    
    return {"token": token_str, "id": token_id, "text": full_text, "done": done}
...
def api_analyze():
    """Run forward pass. Returns predictions + analysis. Does NOT commit a token."""
    if session["input_ids"] is None:
        return jsonify({"error": "No session. Send a message first."}), 400
    if session.get("_pending_logits") is not None:
        return jsonify({"error": "Already analyzed. Commit or skip first."})
    result = analyze_step()
...
def api_commit():
    ...
    if token_id is None:
        token_id = session.get("_greedy_id", 0)
    result = commit_token(int(token_id))
    return jsonify(result)
